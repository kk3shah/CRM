...
# ─── Click-Through Tracking (for plain-text emails) ─────────────────────────


@app.route("/click/<tracking_id>")
def click_tracking(tracking_id):
    """
    Logs a click event (equivalent to an open) and redirects to the real dashboard URL.
    Used in plain-text ecom emails where tracking pixels can't render.

    URL format: /click/<tracking_id>?url=<encoded_dashboard_url>
    """
    redirect_url = request.args.get("url", "https://www.dedolytics.org")
