...
# ─── Search Queries ───────────────────────────────────────────────────────────────
TARGET_QUERIES = [
    # Apparel / fashion DTC — repeat purchase and return rate analytics
    "online clothing boutique Toronto",
    "Canadian apparel brand online",
    "DTC fashion brand Canada",
    # Health & wellness ecom — subscription churn, bundle analytics
    "supplements brand Canada online",
    "health products ecommerce Canada",
    "vitamin brand Canada DTC",
    # Beauty — AOV and repurchase triggers
    "beauty brand ecommerce Canada",
    "skincare brand Canada online",
    "cosmetics store online Canada",
    # Home & lifestyle — strong AOV optimization story
    "home decor online store Canada",
    "furniture ecommerce Canada",
    # Pet — high LTV, repeat purchase data
    "pet supplies store Canada online",
    "pet food brand Canada DTC",
    # Fitness & sports ecom
    "fitness equipment online Canada",
    "sports nutrition Canada online",
    # Subscription box — churn is the core pain
    "subscription box Canada",
    "monthly box service Canada",
]
...
def scrape_gta_smbs(target_leads: int = 70) -> dict:
    """
    ...
    try:
        ...
        for query_idx, (query_category, city) in enumerate(query_pairs):
            ...
            for place in places:
                ...
                elif not enrichment.get("ecommerce_detected"):
                    skip_reason = "No ecommerce detected"
                elif enrichment.get("product_count_estimate", 0) <= 10:
                    skip_reason = "Est. product count <= 10"
                elif enrichment.get("ad_pixels_detected") != "yes":
                    skip_reason = "No ad pixels detected"

                # Check for negative keywords in description/name
                text_corpus = (name + " " + description).lower()
                if (
                    "restaurant" in text_corpus
                    or "agency" in text_corpus
                    or "manufacturer" in text_corpus
                    or "b2b" in text_corpus
                    or "cafe" in text_corpus
                ):
                    skip_reason = "Matches negative business type (restaurant/agency/manufacturer)"
...
if __name__ == "__main__":
    scrape_gta_smbs(target_leads=70)
